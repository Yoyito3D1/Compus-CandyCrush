/*------------------------------------------------------------------------------
	...
	Programador tarea 2B: jonjordi.salvado@estudiants.urv.cat
...
void genera_mapa2(char mat[][COLUMNS])
{
	...
	for (i=0; i<ROWS; i++)
	{
		for (j=0; j<COLUMNS; j++)
		{
			if (mat[i][j]==15)	fija_metabaldosa((u16 *) 0x06000800, i, j, 19);
			else{
				if ((i+j)%2==0) fija_metabaldosa((u16 *) 0x06000800, i, j, 17);
				else fija_metabaldosa((u16 *) 0x06000800, i, j, 18);
			}
		}
	}
}
...
// Tareas 2Ba y 2Ca:
	...
	dmaCopy(BaldosasPal, BG_PALETTE, BaldosasPalLen);
